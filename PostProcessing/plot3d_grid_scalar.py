...
#Space averaging. Plot a plane defined by the normal vector of the Cartesian coordinate 'x' or 'y' or 'z'
slice_direction = 'z'

#Choose the position to locate the slice (no interpolation, plots only the closest grid point)
slice_position = 0.0
...
if slice_direction == 'x' or slice_direction == 'X':
	xslice = True
	print_dir += '_xslice'

	if sp < XG[0]:
		print("Slice position is less than xmin. Setting to xmin.")
		sp = XG[0]
	elif sp > XG[-1]:
		print("Slice position is greater than xmax. Setting to xmax.")
		sp = XG[-1]

	islice = np.argmin(np.abs(sp - XG))
	print("Plotting slice in the y-z plane at x = %.4e\n" % (XG[islice]))

elif slice_direction == 'y' or slice_direction == 'Y':
	yslice = True
	print_dir += '_yslice'

	if sp < YG[0]:
		print("Slice position is less than ymin. Setting to ymin.")
		sp = YG[0]
	elif sp > YG[-1]:
		print("Slice position is greater than ymax. Setting to ymax.")
		sp = YG[-1]

	islice = np.argmin(np.abs(sp - YG))
	print("Plotting slice in the x-z plane at y = %.4e\n" % (YG[islice]))

elif slice_direction == 'z' or slice_direction == 'Z':
	zslice = True
	print_dir += '_zslice'

	if sp < ZG[0]:
		print("Slice position is less than zmin. Setting to zmin.")
		sp = ZG[0]
	elif sp > ZG[-1]:
		print("Slice position is greater than zmax. Setting to zmax.")
		sp = ZG[-1]

	islice = np.argmin(np.abs(sp - ZG))
	print("Plotting slice in the x-y plane at z = %.4e\n" % (ZG[islice]))
else:
	print("Slice direction %s is not recognised. Choose \'x\' or \'y\' or \'z\'\nEXITING\n" % (slice_direction))
	sys.exit()
...
for sn in slist:
	...
	if (xslice):
		DAT_tavg = np.zeros([nz,ny])
	elif(yslice):
		DAT_tavg = np.zeros([nz,nx])
	else:
	...
	for i in range(ntotal):
		...
		#Loading the data
		if (species):
			fname = "Output/" + data_dir + "/s%01d_%08d.bin" % (sn,tlist[i])
		else:
			fname = "Output/" + data_dir + "/%08d.bin" % (tlist[i])

		DAT = np.fromfile(fname,dtype=np.double,count=nx*ny*nz)

		#Reshaping the data
		DAT = np.reshape(DAT,(nz,ny,nx))

		#Handling periodic edges of domain
		if (PER[0] == 0):
			xface0 = np.array(DAT[:,:,0])
			xface1 = np.array(DAT[:,:,-1])
			DAT[:,:,-1] = xface0

		if (PER[1] == 0):
			yface0 = np.array(DAT[:,0,:])
			DAT[:,-1,:] = yface0

		if (PER[2] == 0):
			zface0 = np.array(DAT[0,:,:])
			DAT[-1,:,:] = zface0
		...
		if xslice:
			if fixed_levels == False:
				cs = plt.contourf(YG, ZG, DAT, nlevels)
			if fixed_levels == True:
				cs = plt.contourf(YG, ZG, DAT, levels, extend='both')
		...
		if yslice:
			if fixed_levels == False:
				cs = plt.contourf(XG, ZG, DAT, nlevels)
			if fixed_levels == True:
				cs = plt.contourf(XG, ZG, DAT, levels, extend='both')
		...
		if zslice:
			if fixed_levels == False:
				cs = plt.contourf(XG, YG, DAT, nlevels)
			if fixed_levels == True:
				cs = plt.contourf(XG, YG, DAT, levels, extend='both')
	...
	if (time_average):
		...
		if xslice:
			if fixed_levels == False:
				cs = plt.contourf(YG, ZG, DAT_tavg, nlevels)
			if fixed_levels == True:
				cs = plt.contourf(YG, ZG, DAT_tavg, levels, extend='both')
		...
		if yslice:
			if fixed_levels == False:
				cs = plt.contourf(XG, ZG, DAT_tavg, nlevels)
			if fixed_levels == True:
				cs = plt.contourf(XG, ZG, DAT_tavg, levels, extend='both')
		...
		if zslice:
			if fixed_levels == False:
				cs = plt.contourf(XG, YG, DAT_tavg, nlevels)
			if fixed_levels == True:
				cs = plt.contourf(XG, YG, DAT_tavg, levels, extend='both')
